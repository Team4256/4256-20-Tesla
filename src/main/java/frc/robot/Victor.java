...
public class Victor extends VictorSPX {
	...
	/**
	 * This function prepares a motor by setting the minimum and maximum
	 * percentages.
	 **/
	...
	/**
	 * This function sets the motor's output based on the control mode. Percent: -1
	 * to 1
	 **/
	...
	public void set(final double value, final boolean updateSetPoint) throws IllegalAccessException {
		...
		case Current:
			throw new IllegalAccessException(
					"Victor " + Integer.toString(getDeviceID()) + "'s mode is incompatible with Victors");
		case Follower:
			break;
		case PercentOutput:
		...
		case Disabled:
			break;
		default:
			throw new IllegalAccessException("Victor " + Integer.toString(getDeviceID()) + "'s mode is unimplemented.");
		}
	...
	private double setPercent(final double percentage) throws IllegalAccessException {
		if (controlMode == percent) {
			...
			throw new IllegalAccessException("Victor " + Integer.toString(getDeviceID()) + " was given percentage in "
					+ controlMode.name() + " mode.");
		}
		return percentage;
	}
	...
	/**
	 * Run this after all other commands in a system level loop to make sure the
	 * Talon receives a command.
	 **/
	public void completeLoopUpdate() {
		if (!updated) {
			if (lastSetpoint != null)
				super.set(controlMode, lastSetpoint);// send a command if there hasn't yet been one, using raw encoder
														// units
			else
				neutralOutput();
		}
		...
		if (getControlMode() != follower) {
			updated = false;
		} // loop is over, reset updated for use in next loop (followers excluded)
	}
}
