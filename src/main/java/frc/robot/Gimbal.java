...
public class Gimbal {
	...
	public void set(final double degreesX, final double degreesY) {
		x.setAngle(degreesX);
		y.setAngle(degreesY);
	}

	public void increment(final double amountX, final double amountY) {
		x.increment(amountX * K);
		y.increment(amountY * K);
	}

	private static class BetterServo extends Servo {
		private double angle = 0.0;

		public BetterServo(int port) {
			super(port);
		}
